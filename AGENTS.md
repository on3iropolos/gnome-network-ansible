...
1. **At session start**: Run `bd prime` or `bd ready --json` to load beads context
...
7. **Use `bd` for ALL task tracking** - no markdown TODOs
8. **Always use `--json`** flag for programmatic access
9. **Sync at session end**: `bd sync`
